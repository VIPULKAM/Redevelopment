...
def calculate_profit(
    ...
    # REFACTORED: Calculate effective FSI including TDR, fungible FSI or ancillary FSI
    effective_fsi = base_fsi + tdr_bonus
    
    # REFACTORED: Include fungible FSI (Mumbai) or ancillary FSI (other regions) in buildable area calculation
    fungible_area_factor = 0
    ancillary_area_factor = 0
    
    if REGION_CONFIG[region]["has_fungible"] and fungible_fsi > 0:
        # For Mumbai - fungible FSI adds to buildable area
        fungible_area_factor = base_fsi * fungible_fsi
    elif not REGION_CONFIG[region]["has_fungible"] and ancillary_fsi > 0:
        # For other regions - ancillary FSI adds to buildable area
        ancillary_area_factor = base_fsi * ancillary_fsi
    
    # REFACTORED: Update effective FSI with fungible/ancillary additions
    total_effective_fsi = effective_fsi + fungible_area_factor + ancillary_area_factor
    
    # Calculate buildable area with total effective FSI
    ...
    return {
        ...
        "effective_fsi": effective_fsi,  # FSI with TDR only
        "total_effective_fsi": total_effective_fsi,  # FSI with TDR, fungible/ancillary
...
with col2:
    if calculate_button:
        ...
        # REFACTORED: Added more details about FSI calculations
        st.markdown(f"""
        {land_area_text}
        {road_width_text}
        ...
        # REFACTORED: Show fungible or ancillary FSI details
        if REGION_CONFIG[region]["has_fungible"]:
            st.markdown(f"""
        ...
        # REFACTORED: Show total effective FSI
        st.markdown(f"""
        ...
        # REFACTORED: New FSI composition visualization
        with viz_tab3:
